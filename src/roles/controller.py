...
async def create_role(body: RolesCreateSchema, session: AsyncSession) -> RolesResponseSchema:
    ...
    try:
        # Moves the object data to the pending state, until the next flush,
        # at which point they will move to the persistent state.
        session.add(role)
        await session.commit()  # Saves the data into db tables.
        # Updates the object with the fresh data that's bin created in the
        # database and fetches the server-generated created_at back into the object.
...
async def delete_role_by_id(id: str, session: AsyncSession) -> None:
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Cannot delete this role because it's currently assigned to one or more users."
        )
