    ...
    try:
        ...
        existing_balance = await session.scalar(
            select(BalanceModel).where(
                BalanceModel.user_id == user.id)
        )
        if existing_balance:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                                detail="The user have already created a balance.")

        balance_name = body.name
        if not body.name:
            user_name = await session.scalar(select(UsersModel.name).where(
    ...
    try:
        ...
        if not balance:
            raise HTTPException(
                ...
                detail="User balance record not found. Please create a balance profile."
            )
        return balance
...
async def update_balance_by_id(
    ...
    try:
        current_balance = await session.scalar(select(BalanceModel).where(
            BalanceModel.id == id,
            BalanceModel.user_id == user.id
        ))
