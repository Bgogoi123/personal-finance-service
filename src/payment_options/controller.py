...
async def get_all_payment_options(
        session: AsyncSession,
        user: UsersModel
) -> List[PaymentOptionsResponseSchema]:
    try:
        ...
        if not options:
            raise HTTPException(
                ...
                detail="No Payment Options Found."
            )
        return options
...
async def get_payment_option_by_id(
    ...
    try:
        stmt = select(PaymentOptionsModel).where(
            PaymentOptionsModel.id == id,
            PaymentOptionsModel.user_id == user.id)
...
async def update_payment_option_by_id(
    ...
    try:
        ...
        if body.payment_type is not None:
            option.payment_type = body.payment_type
...
async def delete_payment_option_by_id(id: str, session: AsyncSession, user: UsersModel) -> dict:
    if not id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid Option ID."
        )
